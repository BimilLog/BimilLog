import { apiClient } from '../client'
import { Setting, KakaoFriendList, SimpleMember } from '@/types/domains/user'
import { PageResponse } from '@/types/common'

export const userQuery = {
  checkUserName: (memberName: string) =>
    apiClient.get<boolean>(`/api/member/username/check?memberName=${encodeURIComponent(memberName)}`),

  getSettings: () =>
    apiClient.get<Setting>("/api/member/setting"),

  getFriendList: async (page = 0, size = 10) => {
    const response = await apiClient.get<KakaoFriendList>(`/api/member/friendlist?page=${page}&size=${size}`)

    if (!response.success) {
      throw new Error(response.error || '친구 목록을 불러올 수 없습니다.')
    }

    return response
  },

  getAllMembers: (page = 0, size = 10) =>
    apiClient.get<PageResponse<SimpleMember>>(`/api/member/all?page=${page}&size=${size}&sort=createdAt,desc`),

  searchMembers: (keyword: string, page = 0, size = 10) =>
    apiClient.get<PageResponse<SimpleMember>>(`/api/member/search?query=${encodeURIComponent(keyword)}&page=${page}&size=${size}&sort=createdAt,desc`),
}
