...
export const userQuery = {
  ...
  getUserLikedPosts: (page = 0, size = 10) =>
    apiClient.get<PageResponse<SimplePost>>(`/api/user/liked-posts?page=${page}&size=${size}`),
  
  getUserLikedComments: (page = 0, size = 10) =>
    apiClient.get<PageResponse<SimpleComment>>(`/api/user/liked-comments?page=${page}&size=${size}`),
}