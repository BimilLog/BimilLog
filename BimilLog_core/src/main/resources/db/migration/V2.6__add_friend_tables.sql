...
CREATE TABLE IF NOT EXISTS `friend_recommendation` (
    ...
    -- 인덱스 (조회 성능 최적화)
    INDEX `idx_friend_recommendation_member` (`member_id`)
        COMMENT '회원의 친구 추천 목록 조회 시 성능 최적화',

    INDEX `idx_friend_recommendation_recommend` (`recommend_member_id`)
        COMMENT '추천된 회원으로 조회 시 성능 최적화',

    INDEX `idx_friend_recommendation_member_score` (`member_id`, `score` DESC)
        COMMENT '회원의 추천 친구를 점수순으로 조회 시 성능 최적화'

) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='친구 추천 테이블 (점수 기반 추천 시스템, 2촌/3촌 구분)';
