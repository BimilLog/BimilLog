...
CREATE INDEX idx_post_read_model_created ON post_read_model (created_at DESC);
...
CREATE TABLE post_read_model_dlq (
    ...
    CONSTRAINT uk_post_dlq_event_status UNIQUE (event_id, status)
);

CREATE INDEX idx_post_dlq_status_created ON post_read_model_dlq (status, created_at);
...
CREATE INDEX idx_processed_event_type_at ON processed_event (event_type, processed_at);
