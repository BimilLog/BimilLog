...
/**
 * <h2>PostCacheSyncPort</h2>
 * <p>
 * Post 도메인에서 캐시 데이터 동기화를 위한 DB 조회 작업을 처리하는 아웃바운드 포트입니다.
 * 헥사고날 아키텍처에서 캐시 시스템과 데이터베이스 간 데이터 동기화의 추상화된 인터페이스 역할을 합니다.
 * </p>
 * <p>PostCacheService의 스케쥴러가 정기적으로 인기글 캐시를 갱신할 때 호출됩니다.</p>
 * <p>실시간(1일), 주간(7일), 레전드(추천 20개 이상) 인기글 데이터를 DB에서 조회하여 Redis 캐시 업데이트용 데이터를 제공합니다.</p>
 * <p>캐시 무효화나 강제 동기화가 필요한 상황에서도 게시글 상세 정보 조회 기능을 제공합니다.</p>
 *
 * @author Jaeik
 * @version 2.0.0
 */
