...
public interface JwtPort {
     ...
     * <p>{@link AuthCookieManager}에서 로그인 성공 후 액세스 토큰 발급 시 호출됩니다.</p>
     *
     ...
     * @since 2.0.0
     */
    String generateAccessToken(ExistedUserDetail userDetail);

    /**
     * <h3>JWT 리프레시 토큰 생성</h3>
     * <p>사용자 ID와 토큰 ID를 포함한 JWT 리프레시 토큰을 생성합니다. 유효기간은 30일입니다.</p>
     * <p>{@link AuthCookieManager}에서 로그인 성공 후 리프레시 토큰 발급 시 호출됩니다.</p>
     *
     * @param userDetail 클라이언트용 사용자 정보 DTO
     ...
     * @since 2.0.0
     */
    String generateRefreshToken(ExistedUserDetail userDetail);
     ...
     * @since 2.0.0
     */
    ExistedUserDetail getUserInfoFromToken(String jwtAccessToken);

    /**
     * <h3>JWT 리프레시 토큰에서 토큰 ID 추출</h3>
     * <p>JWT 리프레시 토큰에서 토큰 ID를 추출합니다.</p>
     * <p>{@link AuthCookieManager}에서 토큰 갱신 시 기존 토큰 ID를 조회하기 위해 호출됩니다.</p>
     *
     ...
     * <p>{@link AuthCookieManager}에서 토큰 갱신 시 리프레시 토큰 재발급 여부 결정을 위해 호출됩니다.</p>
     *
