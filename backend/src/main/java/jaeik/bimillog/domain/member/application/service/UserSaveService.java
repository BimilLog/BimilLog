...
import jaeik.bimillog.domain.member.application.port.in.UserSaveUseCase;
...
public class UserSaveService implements UserSaveUseCase {
    ...
    public MemberDetail processUserData(SocialProvider provider, SocialUserProfile userProfile) {
        Optional<Member> existingUser = userQueryPort.findByProviderAndSocialId(provider, userProfile.getSocialId());
        if (existingUser.isPresent()) {
            Member member = existingUser.get();
            return saveUserPort.handleExistingUserData(member, userProfile);
        } else {
