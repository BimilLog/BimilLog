    ...
    private JPAQuery<PostSearchResult> createBasePopularPostsQuery() {
        ...
        return jpaQueryFactory
                .select(Projections.constructor(PostSearchResult.class,
                        ...
                        post.postCacheFlag,                  // 5. postCacheFlag (PostCacheFlag)
                        post.createdAt,                      // 6. createdAt (Instant)
                        member.id,                           // 7. memberId (Long)
                        member.memberName,                   // 8. memberName (String)
                        comment.countDistinct().intValue(),  // 9. commentCount (Integer)
                        post.isNotice))                      // 10. isNotice (boolean)
                .from(post)
