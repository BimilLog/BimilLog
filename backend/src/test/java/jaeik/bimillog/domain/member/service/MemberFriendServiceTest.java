...
class MemberFriendServiceTest extends BaseUnitTest {
    ...
    void shouldGetKakaoFriendList_WhenValidRequest() {
        ...
        given(globalAuthTokenQueryPort.findById(tokenId)).willReturn(Optional.of(authToken));
        ...
        // When
        KakaoFriendsResponseVO result = memberFriendService.getKakaoFriendList(1L, tokenId, 0, 3);
        ...
        verify(memberQueryPort).findMemberNamesInOrder(Arrays.asList("1", "2", "3"));
    }
    ...
    void shouldThrowException_WhenTokenNotFound() {
        // Given
        given(globalAuthTokenQueryPort.findById(1L)).willReturn(Optional.empty());
        ...
        verify(globalAuthTokenQueryPort).findById(1L);
    }
