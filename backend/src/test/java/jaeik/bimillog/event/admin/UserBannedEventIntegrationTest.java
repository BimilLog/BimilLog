...
public class UserBannedEventIntegrationTest extends BaseEventIntegrationTest {
    ...
    void multipleUserBannedEventsForSameUser_ShouldProcessAll() {
        ...
        SocialProvider provider = SocialProvider.KAKAO;
        UserBannedEvent event1 = EventTestDataBuilder.createUserBannedEvent(userId, socialId, provider);
        UserBannedEvent event2 = EventTestDataBuilder.createUserBannedEvent(userId, socialId, provider);
        UserBannedEvent event3 = EventTestDataBuilder.createUserBannedEvent(userId, socialId, provider);

        // When & Then - 동일 사용자에 대한 차단 이벤트 여러 번 발행
        publishEventsAndVerify(new Object[]{event1, event2, event3}, () -> {
    ...
    void userBannedEventsWithInvalidData_ShouldThrowException() {
        ...
        assertThatThrownBy(() -> EventTestDataBuilder.createUserBannedEvent(null, "testId", SocialProvider.KAKAO))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("사용자 ID는 null일 수 없습니다");

        assertThatThrownBy(() -> EventTestDataBuilder.createUserBannedEvent(1L, null, SocialProvider.KAKAO))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("소셜 ID는 null이거나 비어있을 수 없습니다");

        assertThatThrownBy(() -> EventTestDataBuilder.createUserBannedEvent(1L, "", SocialProvider.KAKAO))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("소셜 ID는 null이거나 비어있을 수 없습니다");

        assertThatThrownBy(() -> EventTestDataBuilder.createUserBannedEvent(1L, "testId", null))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void eventProcessingWithException_SocialUnlinkFailure() {
        // Given
        UserBannedEvent event = EventTestDataBuilder.createUserBannedEvent(1L, "errorTest", SocialProvider.KAKAO);

        // 소셜 로그인 해제 실패 시뮬레이션
        doThrow(new RuntimeException("소셜 로그인 해제 실패"))
                .when(socialStrategyPort).unlink(SocialProvider.KAKAO, "errorTest");

        // When & Then - 예외가 발생해도 이벤트 리스너는 호출되어야 함
    ...
    void sequentialUserBannedEvents_ShouldMaintainOrder() {
        ...
        Long userId = 1L;
        UserBannedEvent event1 = EventTestDataBuilder.createUserBannedEvent(userId, "first", SocialProvider.KAKAO);
        UserBannedEvent event2 = EventTestDataBuilder.createUserBannedEvent(userId, "second", SocialProvider.KAKAO);
        UserBannedEvent event3 = EventTestDataBuilder.createUserBannedEvent(userId, "third", SocialProvider.KAKAO);

        // When & Then - 순서대로 차단 이벤트 발행
        publishEventsAndVerify(new Object[]{event1, event2, event3}, () -> {
