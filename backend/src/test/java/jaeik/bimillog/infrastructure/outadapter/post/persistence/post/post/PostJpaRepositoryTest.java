...
class PostJpaRepositoryTest {
    ...
    void shouldCheckExistence_WhenValidIdProvided() {
        ...
        // When: 게시글 존재 여부 확인
        boolean exists = postJpaRepository.existsById(savedPost.getId());
        boolean notExists = postJpaRepository.existsById(99999L);
    ...
    void shouldReturnEmpty_WhenNonExistentIdProvided() {
        ...
        // When: 존재하지 않는 ID로 조회
        Optional<Post> result = postJpaRepository.findById(nonExistentId);
    ...
    void shouldSavePost_WhenUserIsNull() {
        ...
        Post savedPost = postJpaRepository.save(anonymousPost);
    ...
    void shouldMaintainNoticeFlag_WhenPostSaved() {
        ...
        // When: 공지사항 저장
        Post savedNotice = postJpaRepository.save(notice);
        entityManager.flush();
        entityManager.clear();

        Post foundNotice = postJpaRepository.findById(savedNotice.getId()).get();
    ...
    void shouldHandleMultiplePosts_WhenProcessingMany() {
        ...
        // Then: 모든 게시글 정상 저장
        long totalCount = postJpaRepository.count();
    ...
    void shouldMaintainConsistency_WhenConcurrentModifications() {
        ...
        Post post1 = postJpaRepository.findById(savedPost.getId()).get();
        ...
        // Then: 최종 수정 사항이 반영됨
        Post finalPost = postJpaRepository.findById(savedPost.getId()).get();
