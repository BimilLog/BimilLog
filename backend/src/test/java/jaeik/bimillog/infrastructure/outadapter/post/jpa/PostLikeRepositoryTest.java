package jaeik.bimillog.infrastructure.outadapter.post.jpa;
...
class PostLikeRepositoryTest {
    ...
    void shouldDeleteAllLikes_WhenValidPostIdProvided() {
        ...
        // 삭제 전 추천 수 확인
        assertThat(postLikeRepository.countByPost(post)).isEqualTo(3L);
        ...
        // Then: 모든 추천이 삭제됨
        assertThat(postLikeRepository.countByPost(post)).isEqualTo(0L);
    }
    ...
    void shouldNotDeleteOtherPostLikes_WhenDeletingSpecificPostLikes() {
        ...
        postLikeRepository.deleteAllByPostId(post1.getId());
        entityManager.flush();

        // Then: 첫 번째는 삭제, 두 번째는 유지
        assertThat(postLikeRepository.countByPost(post1)).isEqualTo(0L);
        assertThat(postLikeRepository.countByPost(post2)).isEqualTo(1L);
    }
    ...
    void shouldHandleGracefully_WhenDeletingNonExistentPostId() {
        ...
        postLikeRepository.deleteAllByPostId(nonExistentPostId);
        entityManager.flush();

        // Then: 예외 없이 정상 처리 (아무것도 삭제되지 않음)
        // 기존 데이터가 없으므로 카운트는 여전히 0
        List<PostLike> allLikes = postLikeRepository.findAll();
    ...
    void shouldReturnFalse_WhenUserOrPostIsNull() {
        ...
        boolean nullUserExists = postLikeRepository.existsByUserAndPost(null, post);
        boolean nullPostExists = postLikeRepository.existsByUserAndPost(user, null);
        boolean bothNullExists = postLikeRepository.existsByUserAndPost(null, null);
    ...
    void shouldReturnZero_WhenPostIsNullForCount() {
        ...
        long count = postLikeRepository.countByPost(null);
    ...
    void shouldPreventDuplicateLikes_WhenSameUserLikesSamePostTwice() {
        ...
        PostLike secondLike = PostLike.builder()
                ...
                .build();
        try {
            postLikeRepository.save(secondLike);
            entityManager.flush();
    ...
    void shouldHandleMultipleLikes_WhenProcessingMany() {
        ...
        // Then: 모든 추천이 정상 처리됨
        long totalCount = postLikeRepository.countByPost(popularPost);
        assertThat(totalCount).isEqualTo(LIKE_COUNT);

        // 조회 성능 확인
        long startTime = System.currentTimeMillis();
        boolean randomUserLikeExists = postLikeRepository.existsByUserAndPost(
    ...
    void shouldMaintainConsistency_BetweenDeleteAndQuery() {
        ...
        // 초기 상태 확인
        assertThat(postLikeRepository.countByPost(post)).isEqualTo(3L);
        assertThat(postLikeRepository.existsByUserAndPost(user1, post)).isTrue();
        assertThat(postLikeRepository.existsByUserAndPost(user2, post)).isTrue();
        assertThat(postLikeRepository.existsByUserAndPost(user3, post)).isTrue();

        // When: 한 명의 추천 삭제
        postLikeRepository.deleteByUserAndPost(user2, post);
        entityManager.flush();

        // Then: 삭제된 사용자는 false, 나머지는 true, 카운트는 2
        assertThat(postLikeRepository.countByPost(post)).isEqualTo(2L);
        assertThat(postLikeRepository.existsByUserAndPost(user1, post)).isTrue();
        assertThat(postLikeRepository.existsByUserAndPost(user2, post)).isFalse();
        assertThat(postLikeRepository.existsByUserAndPost(user3, post)).isTrue();
    }
